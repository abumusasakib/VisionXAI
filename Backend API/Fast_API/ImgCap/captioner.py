...
# Setup loguru logger
logger.add("captioner_{time}.log", rotation="1 day", retention="7 days", compression="zip")
...
# Loading weights
weights_filepath = f"{dx}imgcap_{mdx}"
# Load weights if available
load_weights(f"{dx}imgcap_{mdx}")
...
# Vocabulary set
try:
    vx = pickle.load(open(f'{dx}vocab_{mdx}', 'rb'))
    index_lookup = dict(zip(range(len(vx)), vx))
except Exception as e:
